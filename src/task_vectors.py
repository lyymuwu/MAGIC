...
class TaskVector():
    def __init__(self, pretrained_checkpoint=None, finetuned_checkpoint=None, 
                 vector=None, cut_add=False, decompose=False, zero_init=False):
    ...
    def save2ckpt(self, pretrained_checkpoint, output_checkpoint, scaling_coef=1.0):
        """Save the task vector applied to a pretrained model as a checkpoint."""
        with torch.no_grad():
        ...
        torch.save(pretrained_model, output_checkpoint)
        print(f'Saved merged checkpoint to {output_checkpoint}')
        ...
        with torch.no_grad():
            ...
            for key in pretrained_state_dict:
                ...
                if concat and 'c_proj.weight' in key:
                    new_state_dict[key] = (pretrained_state_dict[key] + scaling_coef * self.vector[key]) / 2
                else:
                    new_state_dict[key] = pretrained_state_dict[key] + scaling_coef * self.vector[key]
